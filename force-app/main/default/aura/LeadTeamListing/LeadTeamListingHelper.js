({ 
    submitHelper: function(component,event,helper) 
    {
        ...
        var leadTeamsInForm = component.get("v.leadTeams");       
        if(res)
        {
            ...
            if(allValid == true){
                ...
                {
                    var leadTeam = {};                   
                    leadTeam.Team_Member__c = MemberField.get("v.value");
                    leadTeam.Team_Role__c = RoleField.get("v.value");
                    leadTeam.Lead_Access__c = AccessField.get("v.value");
                    leadTeam.Lead__c = component.get("v.parentRecordId");
                    leadTeams.push(leadTeam);                    
                }
                else
                {
                    res.forEach(function(v,i) {
                        //v.submit();
                        var leadTeam = {};                   
                        leadTeam.Team_Member__c = MemberField[i].get("v.value");
                        leadTeam.Team_Role__c = RoleField[i].get("v.value");
                        leadTeam.Lead_Access__c = AccessField[i].get("v.value");
                        leadTeam.Lead__c = component.get("v.parentRecordId");
                        leadTeams.push(leadTeam);
                        /*
                        helper.showToast("success","Success","Add Lead Team Members Sucessfully");
                ...
                var action = component.get("c.createOrUpdateLeadTeam");
                action.setParams({ leadTeams : leadTeams });
                        ...
                        if(context != undefined) 
                        {
                            if(context == 'Theme4t' || context == 'Theme4d') 
                            {
                                ...
                                sforce.one.showToast({         
                                    "message": "Add Lead Team Members Sucessfully",
                                    "type": "success"
                                });                        
    ...
    {
        var validCase = true;
        var allValid = component.find('leadTeamField');
       ...
       /* var validCase = true;
        var allValid = component.find('leadTeamField').reduce(function (validFields, inputCmp) {
            console.log(allValid);
            inputCmp.showHelpMessageIfInvalid();
