({ 
    submitHelper: function(component,event,helper) 
    {
        ...
        var OrderTeamsInForm = component.get("v.OrderTeams");       
        if(res)
        {
            ...
            if(allValid == true){
                ...
                {
                    var OrderTeam = {};                   
                    OrderTeam.Team_Member__c = MemberField.get("v.value");
                    OrderTeam.Team_Role__c = RoleField.get("v.value");
                    OrderTeam.Order_Access__c = AccessField.get("v.value");
                    OrderTeam.Order__c = component.get("v.parentRecordId");
                    OrderTeams.push(OrderTeam);                    
                }
                else
                {
                    res.forEach(function(v,i) {
                        //v.submit();
                        var OrderTeam = {};                   
                        OrderTeam.Team_Member__c = MemberField[i].get("v.value");
                        OrderTeam.Team_Role__c = RoleField[i].get("v.value");
                        OrderTeam.Order_Access__c = AccessField[i].get("v.value");
                        OrderTeam.Order__c = component.get("v.parentRecordId");
                        OrderTeams.push(OrderTeam);
                        /*
                        helper.showToast("success","Success","Add Order Team Members Sucessfully");
                ...
                var action = component.get("c.createOrUpdateOrderTeam");
                action.setParams({ OrderTeams : OrderTeams });
                        ...
                        if(context != undefined) 
                        {
                            if(context == 'Theme4t' || context == 'Theme4d') 
                            {
                                ...
                                sforce.one.showToast({         
                                    "message": "Add Order Team Members Sucessfully",
                                    "type": "success"
                                });                        
    ...
    {
        var validCase = true;
        var allValid = component.find('OrderTeamField');
       ...
       /* var validCase = true;
        var allValid = component.find('OrderTeamField').reduce(function (validFields, inputCmp) {
            console.log(allValid);
            inputCmp.showHelpMessageIfInvalid();
