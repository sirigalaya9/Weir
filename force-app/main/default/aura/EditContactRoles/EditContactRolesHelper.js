({
        ...
        if (res){
            ...
            if(allValid == true){
                ...
                if(res instanceof Array){
                    ...
                    var projectContactRole = {}; 
                    projectContactRole.Id = IdField.get("v.value");
                    projectContactRole.Contact__c = ContactField.get("v.value");
                    projectContactRole.Role__c = RoleField.get("v.value");
                    projectContactRole.Is_Primary__c = isPrimaryField.get("v.value");
                    projectContactRole.Project__c = component.get("v.parentRecordId");
                    projectContactRoles.push(projectContactRole);
                }
                
                
                var action = component.get("c.createOrUpdateProjectContactRole");
                action.setParams({ projectContactRoles : projectContactRoles });
        		action.setCallback(this, function(response)
				{
                    ...
                    if (state === "SUCCESS") 
                    {
                        if(context != undefined) 
                        {
                            if(context == 'Theme4t' || context == 'Theme4d') 
                            {
                                ...
                                sforce.one.showToast({         
                                    "message": "Edit Project Contact Roles Sucessfully",
                                    "type": "success"
                                });                        
                        ...
                        else 
                        {
                            ...
                            helper.showToast("success","Success","Edit Project Contact Roles Sucessfully");
            ...
            helper.showToast("success","Success","Add Project Contact Roles Sucessfully");
            var viewRecordEvent = $A.get("e.force:navigateToURL");
            viewRecordEvent.setParams({
                "url": "/" + component.get("v.parentRecordId")
            });
    ...
    getProjectContactRoles: function(component, event) 
    {
        component.set("v.isLoading", true);
        var parentRecordId = component.get("v.parentRecordId");
        
        var action = component.get("c.getProjectContactRoles");
        action.setParams({ 
